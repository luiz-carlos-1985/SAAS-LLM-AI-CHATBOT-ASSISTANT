...
The architecture of this system was designed to solve a classic concurrency problem that occurs when trying to combine heavy synchronous operations, such as LangChain agent calls, with the asynchronous nature of FastAPI and real-time streaming via Server-Sent Events.
The flow begins at the front-end built with React 19, which opens a persistent, read-only HTTP connection pointing to the FastAPI streaming endpoint. When the user sends a message, the back-end receives this request and needs to trigger LangChain's AgentExecutor. Since the agent's decision-making process and tool invocations run synchronously by default, executing them directly within FastAPI's main event loop would freeze the entire server for other users.
To bypass this bottleneck, I adopted a decoupling strategy using threads and queues. FastAPI offloads the agent execution to a secondary background thread. Within this thread, I configured a custom callback handler connected to LangChain. Every time the agent takes an action—such as initiating a thought process, triggering a tool, or receiving context from the vector database, this callback captures the event and pushes it into an asynchronous asyncio queue using the call_soon_threadsafe function to guarantee thread-safe communication.
While the agent thread works in isolation, the main FastAPI loop monitors this queue asynchronously. As soon as an event appears in the queue, it is immediately processed and sent to the client as a JSON-formatted SSE event. This cycle repeats continuously until the agent finishes its reasoning loop and places a finalization token in the queue, allowing the final response to render and the connection to close cleanly.
At the data layer, storage operates in an embedded manner. When the RAG pipeline is triggered via the knowledge retrieval tool, it performs a similarity search against ChromaDB, which is persisted locally on disk. To provide system flexibility, I implemented a mechanism that detects if there has been a change in the AI provider chosen by the user in the interface. If the user switches from OpenAI to Groq, for example, the system identifies that the embedding model dimensions are incompatible, automatically clears the local directory, and rebuilds the vector index from scratch with the corresponding model to prevent dimensionality mismatch failures.

The choice of each technology in this project was guided by the need to balance system performance with the inherent complexity of real-time artificial intelligence workflows. Starting with the back-end, the decision to use Python was the most logical path due to the maturity of the AI ecosystem, but the specific choice of FastAPI was due to its native asynchronous architecture. Since the main requirement was to keep connections open for agent event streaming, a traditional blocking framework would quickly deplete server resources. FastAPI made it possible to manage the event loop with high efficiency, isolating the synchronous and heavy processing of the agent into secondary threads, while communication with the client remained fully fluid and non-blocking.
For the orchestration of artificial intelligence, I adopted LangChain. The architectural justification here is standardization and abstraction. The framework offers a cohesive interface for coupling tools and building the agent's routing logic. More importantly, it facilitated the implementation of a provider-agnostic design. I built the system to switch dynamically between OpenAI, when the focus is on more complex reasoning, and Groq using Llama, for tasks that require extremely low-latency inference. This duality was a decision focused on resilience and operational cost optimization, ensuring that the system was not born with vendor lock-in.
In the data layer and the RAG pipeline, I opted for ChromaDB running in an embedded manner. For the current phase of the project, a local vector database solves the semantic search problem with zero infrastructure overhead, eliminating network latency and the need to manage external third-party services. The architecture was designed in a modular way, so, considering a massive scalability like the one we have at iFood, replacing this embedded database with a robust cloud solution, such as a pgvector cluster or Pinecone, would be completely painless and transparent to the application layer.
On the front-end, the choice of React with Vite was guided by reactivity. The observability panel requires the interface to handle a very high volume of state updates in short periods of time as agent events arrive. And for the real-time transport layer, I defended the use of Server-Sent Events over WebSockets. The inference communication of an LLM is strictly unidirectional after the prompt is sent, meaning the client asks and only the server responds with a continuous stream of data. SSE solves exactly this problem by running over the standard HTTP protocol, with less infrastructure complexity, native reconnection, and without the overhead of keeping bidirectional channels open unnecessarily.
